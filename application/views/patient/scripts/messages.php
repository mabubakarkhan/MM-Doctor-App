...
$(document).on('click', '.chat-user', function(event) {
	...
	$.post('<?=BASEURL."patient/get-chat"?>', {id: $this.attr('data-id'), doctor: $this.attr('data-doctor')}, function(resp) {
		resp = $.parseJSON(resp);
		$("#chatArea").html(resp.html);
		$("#chatGroupArea").html(resp.groups);
		$("#chatArea input[name='group']").val($this.attr('data-id'));
		$("#chatArea input[name='doctor']").val($this.attr('data-doctor'));
...
$(document).on('click', '#chatSendBtn', function(event) {
	...
	if ($text.length > 0) {
		...
		$.post('<?=BASEURL."patient/post-message"?>', {
			text: $text,
			group: $("#chatArea input[name='group']").val(),
			doctor: $("#chatArea input[name='doctor']").val()
		}, function(resp) {
			resp = $.parseJSON(resp);
			$("#newChatTag").remove();
...
setInterval(function()
{
	...
	if (typeof($id) != "undefined" && $id !== null) {
	    $.post('<?=BASEURL."patient/new-messages-auto"?>', {
	    	last_id: $id,
			group: $("#chatArea input[name='group']").val(),
			doctor: $("#chatArea input[name='doctor']").val()
	    }, function(resp) {
...
setInterval(function()
{
	...
	if (!($chkSearch.length > 0)) {
	    $.post('<?=BASEURL."patient/groups-list-auto"?>', {
			group: $("#chatArea input[name='group']").val()
	    }, function(resp) {
			resp = $.parseJSON(resp);
			$("#chatGroupArea").html(resp.groups);
