...
<script>
	...
	$(document).on('submit', '#payment-form', function(event) {
		event.preventDefault();
		ajaxBtnLoader($("#payment-form-btn"));
		if ($('input[name="payment_method"]:checked').val() == 'online') {
			...
			function stripeResponseHandler(status, response) {
				if (response.error) {
					...
					$("#payment-form-btn").html('Confirm and Pay');
				} else {
					var token = response['id'];
					$form.find('input[type=text]').empty();
					$('#stripe_token').val(token);
					msgS('Payment in process... please wait.');
					$.post("<?=BASEURL.'submit-cart-checkout'?>", {data: $("#payment-form").serialize()}, function(resp) {
						resp = $.parseJSON(resp);
						if (resp.status = true) {
							msgS(resp.msg);
							$("#payment-form-btn").html('redirecting...');
							window.location.replace("<?=BASEURL?>patient/dashboard");
						}
						else{
							msgE(resp.msg);
							$("#payment-form-btn").html('Submit');
						}
					});//post
				}
			}
