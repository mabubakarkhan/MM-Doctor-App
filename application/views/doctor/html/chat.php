<div class="chat-header">
	...
	<div class="media d-flex">
		<div class="media-img-wrap flex-shrink-0">
			<div class="avatar">
				<img src="<?=UPLOADS.$resp['patient']['img']?>" alt="<?=$resp['patient']['fname'].' '.$resp['patient']['lname']?>" class="avatar-img rounded-circle">
			</div>
		</div>
		<div class="media-body flex-grow-1">
			<div class="user-name"><?=$resp['patient']['fname'].' '.$resp['patient']['lname']?></div>
		...
		<ul class="list-unstyled">
			...
			<?php foreach ($resp['chat'] as $key => $chat): ?>
				<?php if ($newMsgChk == false): ?>
					<?php if ($chat['status'] == 'new' && $chat['sender'] == 'patient'): ?>
						<li class="chat-date" id="newChatTag">new</li>
						<?php $newMsgChk = true; ?>
					<?php endif ?>
				<?php endif ?>
				<li class="media <?=($chat['sender'] == 'patient' ? 'received' : 'sent')?> d-flex">
					<div class="media-body flex-grow-1">
						<div class="msg-box">
							<div>
								<?=$chat['msg']?>
